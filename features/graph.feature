Feature: Graph
  	...
  	Scenario: When user toggles account in account list corresponding line appears in the graph
	...
	Scenario: When a user adds a new line to the graph it is a different color from all the other lines
		...
		And the account is not toggled 
		Then the graph line for that account should show up. 
		And it should be a different color than all the other

	Scenario: When user views the page the graph line color will match the corresponding account button color
		Given user is logged in
			|username|cucumber|
			|password|cucumber|
		And the account is not toggled 
	...
	Scenario: When user views the dash there will be a liability line, network line, and assets line on the graph
		Given user is logged in
			|username|cucumber|
			|password|cucumber|
